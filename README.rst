...
Reflectivity is an API inspired by `Reflectivity in Pharo
...
.. code-block:: python
    ...
    link = reflectivity.MetaLink(MetaLogger(), selector='log_me', control='before')

    # We get the method AST we want to instrument
    rf_ast = reflectivity.reflective_method_for(ExampleClass, 'foo')
...
This small code example uses the two main Reflectivity concepts:
...
The MetaLink ``link`` is used to install a new behavior ``before`` the code of
the 
